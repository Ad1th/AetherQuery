...
def build_sample_query(
    ...
    select_list = (
        ", ".join(parsed.projection_columns)
        if parsed.projection_columns
        else "1 AS __aqp_count_marker"
    )

    sample_clause = _sample_clause(
        source,
        sample_fraction,
    )
    ...
    if parsed.where_clause:
        where_parts.append(
            f"({parsed.where_clause})"
        )

    if source == "mysql":
        where_parts.append(
            f"(RAND() < {sample_fraction:.8f})"
        )

    if sample_clause:
        query = (
            f"SELECT {select_list} "
            f"FROM (SELECT * FROM {parsed.table} "
            f"{sample_clause}) sampled_source"
        )
    else:
        query = (
            f"SELECT {select_list} "
...
def fetch_aggregated_sample(
    ...
    """
    Execute sampled aggregation directly inside
    the source engine.

    Avoids materializing large sampled datasets
    into pandas before aggregation.
    """

    if not parsed.aggregates:
        raise ValueError(
            "fetch_aggregated_sample requires aggregate query metadata"
        )
    ...
    if source == "duckdb":
        ...
        table_reference = parsed.table
        if parsed.table_alias:
            table_reference += f" {parsed.table_alias}"

        if sample_clause:
            from_clause = (
                f"(SELECT * FROM {parsed.table} "
                f"{sample_clause}) "
                f"{parsed.table_alias or 'sampled_source'}"
            )
        else:
    ...
    for aggregate in parsed.aggregates:
        if aggregate.is_count_star:
            select_parts.append(
                f"COUNT(*) AS {aggregate.alias}"
            )
        else:
            select_parts.append(
                f"{aggregate.func.upper()}"
