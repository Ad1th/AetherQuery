...
The `balanced` / `fast` / `accurate` modes stop sampling once the estimate is
within the caller's accuracy target, and fall back to an exact scan otherwise.
Reproduce the evaluation with `scripts/reproduce.sh [SF]` or
`docker build -t aq . && docker run --rm aq`.

Key modules: `backend/core/sufficient_stats.py`,
`backend/core/runtime_sampling.py`, `backend/core/join_sampling.py`,
`backend/stats/` (estimator library, `pytest`).
